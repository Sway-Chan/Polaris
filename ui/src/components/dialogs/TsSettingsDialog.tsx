 ...
 *
 * 本层**最长表单**（主机名 / 出口节点选择 / 接入模式 / 子网路由两向 / 高级折叠）——footer 固定
...
function TsSettingsForm({ node }: { node?: ServerConfig }) {
  ...
  const visibleMain = spec.filter((f) => !f.when || f.when(draft));
  // 高级区同样过 `when`（此前 ADV_SPEC 里无条件字段故直接 map）—— acceptDefaultResolvers 受
  // resolveByName 门控，漏了这一步它会恒显，联动就只剩注释。
  const visibleAdv = ADV_SPEC.filter((f) => !f.when || f.when(draft));
