...
describe('门 3 · 分批接线', () => {
   ...
   * 卡高，而采样器②只看容器盒子、内容变矮不改它。该处 transition 现已收窄到 border-color/
   * box-shadow/background/outline（screens.css:77，均不参与盒模型），`view` 这一维今天不再触发
   * 这个洞，但③守的是机制本身（任何带 CSS 过渡的内容高度变化），不是这一个实例，故本条继续钉
   * 「监听确实挂着、且 cleanup 里摘掉」。
   */
