 ...
 *
 * 复活它的方向不是「后端补一句 `format!`」，理由是**「不再新增后端产出的用户可见文案」**：本仓
 * 已经有一条登记在案的欠账（`emit_progress` 的 `message` 携带硬编码中文、经 `update:progress`
 * 原样广播、绕过 i18n），后端每多拼一个给人看的串，那条口子就宽一格。数字过线、渲染端拼串，
 * 是唯一不扩大它的方向。
 *
 ...
 * 全长路径要 533–747px，溢出 1.5–2.1 倍。而 `.sub.path` 是**尾截断**省略号 ⇒ 屏幕上留下的恰好是
 * 恒定的 `<cache>/updates` 前缀，**「下的是哪个包」那一半被永久裁掉**，对用户零信息量。
 *
