...
mod tests {
    ...
    ///    变异会**全绿**。剩下的残留面：`FLOOR` 是人写的常量，**装了比它更小的项时不会自动下调**
    ///    （门槛此时由 `boxed_min` 接管，仍正确，只是 `FLOOR` 这条兜底会落后于事实）。
